...
export const useDeleteBlockMutation = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            try {
                // Delete the block
                await BlockService.deleteBlock(blockId, controller.signal);

                // Clean up on success
                controllersMapRef.current.delete(blockId);

                return { blockId, documentId };
            } catch (error) {
                // Only clean up if not an abort error (we want to keep the reference for aborted operations)
                if (
                    !(
