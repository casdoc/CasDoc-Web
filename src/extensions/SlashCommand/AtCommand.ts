...
const extensionName = "atCommand";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
let popup: any;

export const AtCommand = Extension.create({
    name: extensionName,
        ...
        return [
            Suggestion({
                ...
                items: ({ query }: { query: string }) => {
                    const withFilteredComponents = COMPONENTS.map(
                        (component) => ({
                            ...component,
                    ...
                    const withoutEmptyComponents =
                        withFilteredComponents.filter(
                            (component) => component.commands.length > 0
                        );

                    const withEnabledSettings = withoutEmptyComponents.map(
                        (component) => ({
                            ...component,
                            commands: component.commands.map((command) => ({
                                ...command,
                                isEnabled: true,
                            })),
                        })
